...
import { ReplicableConfigInterface } from '../../../../shared/globalInterfaces/replicable.interface';

@Injectable()
export class ResultIpExpertisesRepository
  extends Repository<ResultIpExpertises>
  implements ReplicableInterface<ResultIpExpertises>
{
  constructor(
  ...
  private readonly _logger: Logger = new Logger(
    ResultIpExpertisesRepository.name,
  );

  async replicable(
    config: ReplicableConfigInterface<ResultIpExpertises>,
  ): Promise<ResultIpExpertises[]> {
    let final_data: ResultIpExpertises[] = null;
    try {
      if (config.f?.custonFunction) {
        ...
        ,rie3.result_ip_expert_id
         ...
         WHERE rie.is_active > 0
         	AND rie2.result_id = ?
        `;
        const response = await (<Promise<ResultIpExpertises[]>>(
        ...
        const response_edit = <ResultIpExpertises[]>(
