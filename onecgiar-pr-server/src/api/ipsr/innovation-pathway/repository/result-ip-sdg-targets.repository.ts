...
export class ResultIpSdgTargetRepository
  ...
  implements ReplicableInterface<ResultIpSdgTargets>
{
  constructor(
  ...
  private readonly _logger: Logger = new Logger(
    ResultIpSdgTargetRepository.name,
  );

  async replicable(
    config: ReplicableConfigInterface<ResultIpSdgTargets>,
  ): Promise<ResultIpSdgTargets> {
    let final_data: ResultIpSdgTargets = null;
    try {
      if (config.f?.custonFunction) {
        ...
        const response = await (<Promise<ResultIpSdgTargets[]>>(
        ...
        const response_edit = <ResultIpSdgTargets>(
        ...
        ,rist.result_ip_sdg_target_id
         ...
         WHERE rist.result_by_innovation_package_id = ?
          `;
        const temp = await (<Promise<ResultIpSdgTargets[]>>(
